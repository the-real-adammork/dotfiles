...
Every design that touches authentication, authorization, secrets, credentials, deployment, databases, external services, or environment configuration must classify project posture:

Load `$secrets` and follow its canonical posture, generation, storage, `git-secret`, and verification policy. Do not duplicate or weaken that policy in the design.

The design must summarize the outcome:

- project posture;
- which secrets agents may generate;
- which credentials require escalation;
- how unsafe generated plaintext is ignored, encrypted, masked, and verified before commit.
...
**Project Posture:** <side-project/greenfield | internal/demo | production/customer>
...
| Secret / Credential | Generated By | Storage / Handling | Escalation Required? | Rationale |
| --- | --- | --- | --- | --- |
| <secret> | <agent | human | existing> | <git-secret/env/ignored/plain safe demo> | <yes/no> | <why this matches project posture> |
...
- Check that project posture is classified when secrets, credentials, deployment, or external services are involved.
- Check that generated-vs-human-provided secrets follow the posture policy and include storage/handling guidance.
