...
The supervisor transition handler verifies the transition gate, reads the transition handoff/report, merges or reconciles the phase branch into the run base branch, records the resulting base commit and any merge decisions in `run.yaml`, stops the completed orchestrator pane/session, runs post-merge local verification setup from the handoff/report, reports the localhost URL and smoke-test checklist, and then starts the next phase orchestrator/watchdog or sets `status: complete` when no phases remain.
