...
- The supervisor verifies the transition gate and fast-forwards `impl/<phase-slug>` into the run base branch recorded in `run.yaml`.
...
- If the run base branch is not an ancestor of the completed phase branch, stop with a supervisor escalation/handoff rather than silently chaining the next phase from the phase branch.
