...
- The orchestrator stops at accepted phase state: all workers merged into the phase branch, acceptance packet written, phase-transition handoff/report written, `phase.yaml` complete, and supervisor inbox requesting `phase_completion`.
- The supervisor verifies the transition gate, classifies any dirty/ad-hoc changes in the run base worktree, and merges or reconciles `impl/<phase-slug>` into the run base branch recorded in `run.yaml`, preferring fast-forward when valid.
...
- The next phase starts from the updated base branch after that merge, completed-orchestrator teardown, local verification setup from the transition handoff/report, and smoke-test report, so phases build on each other in git history.
