...
Author the HTML preview from the markdown SLICES document as a standalone review page. Do not use a deterministic markdown-to-HTML converter such as `pandoc`.

The agent must read `docs/plans/SLICES.md`, preserve the document's sections, tables, links, paths, status, and approval-preview metadata, and create `docs/plans/SLICES.html` as hand-authored HTML with embedded CSS.

Preview requirements:

- Use semantic HTML: `main`, `section`, headings in order, tables with `thead` and `tbody`.
- Use a constrained readable layout with generous page padding and a maximum content width.
- Make wide planning tables usable with horizontal scroll wrappers, sticky table headers where practical, tabular numbers, and clear cell borders.
- Use a restrained utility-document visual system: white or near-white content surface, high-contrast text, muted metadata, compact badges for status-like values, and no decorative hero treatment.
- Keep all CSS inline in the HTML file so the preview is portable.
- Include a short generated timestamp and the source markdown path near the top.
- Do not omit rows or collapse table content into summaries; the HTML is an approval artifact, not a recap.
...
- Generated by: agent-authored standalone HTML from `docs/plans/SLICES.md`
