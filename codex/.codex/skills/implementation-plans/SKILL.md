...
description: Use when the user has a spec, requirements, issue, or approved design and wants a concrete implementation plan before code changes.
---

# Implementation Plans

Write plans that an agent or engineer can execute without rediscovering the project. Plans should be concrete, testable, scoped to one coherent outcome, and broken into small steps.

Adapted from the Obra/Superpowers `writing-plans` skill for this Codex setup.

## Start

Announce: "I'm using the implementation-plans skill to write the plan."
...
If the request spans independent subsystems, split it into separate plans. Each plan should produce working, testable software on its own.

Do not write a plan with vague umbrella tasks such as "implement backend" or "build UI". Decompose by behavior and ownership boundaries.
...
**Architecture:** <2-3 sentences describing the approach and boundaries>
